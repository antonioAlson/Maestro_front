...
export class JiraService {
  ...
  downloadArquivo(url: string): Observable<Blob> {
    ...
    console.log('📥 [JiraService] Baixando arquivo:', normalizedUrl);
  ...
  private normalizeDownloadUrl(rawUrl: string): string {
    const apiBase = this.apiUrl.replace(/\/api\/?$/, '');

    try {
      const parsed = new URL(rawUrl);
      // Sempre prioriza o host/base configurado no frontend para evitar links com localhost/container.
